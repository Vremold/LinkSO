...
def load_data():
    ...
    chapter_cis = dict()
    chapter_apis = dict()
    with open(CJ2PATHUTIL.textbook_code_element_path, "r", encoding="utf-8") as inf:
        for line in inf:
...
def write_back():
    ...
    package_tfidf = chapter_tf_idf(chapter_packages, save_idf_path=CJ2PATHUTIL.textbook_code_element_idf_path)
    ci_tfidf = chapter_tf_idf(chapter_cis, save_idf_path=CJ2PATHUTIL.textbook_code_element_idf_path)
    api_tfidf = chapter_tf_idf(chapter_apis, save_idf_path=CJ2PATHUTIL.textbook_code_element_idf_path)

    with open(CJ2PATHUTIL.textbook_code_element_path, "r", encoding="utf-8") as inf, open(CJ2PATHUTIL.textbook_filtered_code_element_path, "w", encoding="utf-8") as outf:
        for line in inf:
            splits = line.strip().split("\t")
            chapter = splits[0]
            section = splits[1]
