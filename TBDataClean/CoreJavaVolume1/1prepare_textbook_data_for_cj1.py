...
class TextbookContentExtractor(object):
    def __init__(self, raw_textbook_file, content_file):
        ...
        self.cache_dir = os.path.join(CJ1PATHUTIL.cache_dir)
    ...
    def __extract_content_in_a_section(self, lines, line_idx, chapter_name, curr_chapter_id, section_name, curr_section_id):
        ...
        while line_idx < max_lines:
            ...
            if line.strip() == "Chapter {}".format(curr_chapter_id + 2):
                break
        ...
        if text:
            ...
            if not os.path.exists(save_dir):
                os.mkdir(save_dir)
            with open(os.path.join(save_dir, section_name), "w", encoding="utf-8") as outf:
                outf.write(text)
        return line_idx
    ...
    def extract_content_in_raw_textbook(self):
        ...
        while line_idx < max_lines:
            line = self.__remove_unseen_chars(lines[line_idx])
            # found chapter
            if line.strip() == "Chapter {}".format(expected_chapter_id + 1):
...
class CodeBlockExtractor(object):
    def __init__(self, content_file, dst_dir, dst_code_dir, raw_code_dir):
        ...
        self.cache_dir = CJ1PATHUTIL.cache_dir
    ...
    def load_given_codes(self, raw_code_dir):
        for chapter in os.listdir(raw_code_dir):
            if "v2" in chapter:
                continue
            chapter_idx = int(re.match(r"v1ch(\d+)", chapter).group(1)) - 1
...
class TextbookDataCleaner(object):
    ...
    def extract_paragraph(self, lines, line_idx, section_name, curr_chapter_id, curr_section_id, expected_subsection_id):
        ...
        # next subsetion
        if para and section_name in self.expected_section_to_subsections and expected_subsection_id < len(self.expected_section_to_subsections[section_name]) and "{}.{}.{} {}".format(curr_chapter_id + 1, curr_section_id + 1, expected_subsection_id + 1, self.expected_section_to_subsections[section_name][expected_subsection_id]).startswith(para[0]):
...
if __name__ == "__main__":
    # tbc = TextbookContentExtractor(CJ1PATHUTIL.raw_textbook_path, CJ1PATHUTIL.raw_textbook_contents_path)
    # tbc.extract_content_in_raw_textbook()

    # cbe = CodeBlockExtractor(CJ1PATHUTIL.raw_textbook_contents_path, CJ1PATHUTIL.textbook_dir, CJ1PATHUTIL.textbook_code_dir, CJ1PATHUTIL.raw_textbook_code_dir)
    # cbe.extract_code_blocks()

    tbdc = TextbookDataCleaner(CJ1PATHUTIL.raw_textbook_contents_path, CJ1PATHUTIL.textbook_dir, CJ1PATHUTIL.textbook_code_dir, CJ1PATHUTIL.raw_textbook_code_dir)
